...
def createdir(cwd, dir_name):
    newdir_path = os.path.join(cwd,dir_name)
    if not os.path.exists(newdir_path):
        os.makedirs(newdir_path)
    else:
        i = 1
        while os.path.exists(newdir_path):
            newdir_name = dir_name +'(' + str(i) + ')'
            newdir_path = os.path.join(cwd,newdir_name)
            i += 1
        os.makedirs(newdir_path)
    print('result are put in : ' + newdir_path)
    return newdir_path
...
def training(x_train, x_test, y_train, y_test, workbin, lrate = 0.01, bsize =5, epoch_num = 10):
...
#     model.add(Dropout(0.5))
    ...
    result_bin = createdir(workbin,basic_name)
    pic_name = basic_name + '.png'
    History.loss_plot('epoch',os.path.join(result_bin, pic_name))
    predicted_fc = model.predict(x_test)
    model_name = basic_name + '.h5'
    model.save(os.path.join(result_bin, model_name))
    report_name = basic_name + '.txt'
    predict_y = np.argmax(predicted_fc,axis=1)
    with open(os.path.join(result_bin, report_name),'w') as f:
...
def main():
    ...
    if len(argvs) == 5:
        training(x_train, x_test, y_train, y_test,workbin)
    elif len(argvs) == 6:
        training(x_train, x_test, y_train, y_test,workbin, lrate)
    elif len(argvs) == 7:
        training(x_train, x_test, y_train, y_test,workbin, lrate, bsize)
    elif len(argvs) == 8:
        training(x_train, x_test, y_train, y_test,workbin, lrate, bsize, epoch_num)
